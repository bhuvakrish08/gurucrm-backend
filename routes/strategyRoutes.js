...
// GET /api/strategy/source-mappings - Read all inquiry_lead_source records along with mapping info
  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        CASE WHEN m.strategy_category_id IS NOT NULL AND c.id IS NOT NULL THEN 'Mapped' ELSE 'Unmapped' END AS mapping_status,
        m.updated_at,
        m.updated_by
      FROM inquiry_lead_source s
      LEFT JOIN strategy_source_mappings m ON m.source_id = s.id AND m.is_active = 1
      LEFT JOIN strategy_source_categories c ON c.id = m.strategy_category_id
      WHERE s.status = 1
    `;
    const params = [];
    ...
    if (category_id) {
      query += ` AND m.strategy_category_id = ?`;
      params.push(Number(category_id));
    }

    if (mapping_status === "mapped") {
      query += ` AND m.strategy_category_id IS NOT NULL AND c.id IS NOT NULL`;
    } else if (mapping_status === "unmapped") {
      query += ` AND (m.strategy_category_id IS NULL OR c.id IS NULL)`;
    }

    query += ` ORDER BY mapping_status DESC, s.name ASC`;
    ...
    const [summaryRows] = await db.promise().query(`
      SELECT 
        ...
        SUM(CASE WHEN m.strategy_category_id IS NOT NULL AND c.id IS NOT NULL THEN 1 ELSE 0 END) AS mapped_count,
        SUM(CASE WHEN m.strategy_category_id IS NULL OR c.id IS NULL THEN 1 ELSE 0 END) AS unmapped_count
      FROM inquiry_lead_source s
      LEFT JOIN strategy_source_mappings m ON m.source_id = s.id AND m.is_active = 1
      LEFT JOIN strategy_source_categories c ON c.id = m.strategy_category_id
      WHERE s.status = 1
    ...
    res.json({
      success: true,
      data: rows,
      summary: summaryRows[0] || { total_sources: 0, mapped_count: 0, unmapped_count: 0 }
    });
  } catch (err) {
      ...
      const [affectedQRows] = await db.promise().query(
        `SELECT q.id FROM quotation q
         ...
         WHERE q.quotation_status IN ('Approved', 'Won')
           AND (q.source = ? OR CAST(q.source AS CHAR) = ? OR l.source = ? OR CAST(l.source AS CHAR) = ?)`,
...
router.get("/unmapped-sources", async (req, res) => {
  try {
    const [rows] = await db.promise().query(`
      ...
      LEFT JOIN strategy_source_mappings m ON m.source_id = s.id AND m.is_active = 1
      LEFT JOIN strategy_source_categories c ON c.id = m.strategy_category_id
      LEFT JOIN quotation q ON (q.source COLLATE utf8mb4_unicode_ci = s.name COLLATE utf8mb4_unicode_ci OR q.source COLLATE utf8mb4_unicode_ci = CAST(s.id AS CHAR) COLLATE utf8mb4_unicode_ci) AND q.quotation_status IN ('Approved', 'Won')
      LEFT JOIN (
      ...
      WHERE s.status = 1 AND (m.strategy_category_id IS NULL OR c.id IS NULL)
      GROUP BY s.id, s.name
      ORDER BY affected_quotations_count DESC, s.name ASC
    `);
